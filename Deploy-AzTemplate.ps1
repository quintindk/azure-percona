...
$RandomPubkey = Get-Content ~/.ssh/$AdminUser.pub -Raw
...
if ($UploadArtifacts -Or $useAbsolutePathStaging -or $ArtifactsLocationSasTokenParameter) {
  ...
  if (Test-Path $TemplateParametersFile) {
    # $TempParametersFile = New-TemporaryFile
    # Write-Host $TemplateParameters
    # Set-Content -Path $TempParametersFile -Value $TemplateParameters
    # $JsonParameters = Get-Content $TempParametersFile -Raw | ConvertFrom-Json
    $JsonParameters = $TemplateParameters | ConvertFrom-Json
